...
interface B2Auth { authorizationToken: string; apiUrl: string; downloadUrl: string; }
interface B2Version { fileName: string; fileId: string; }
...
export class B2Storage implements Storage {
  ...
  async write(path: string, body: ReadableStream<Uint8Array>, contentType: string, contentLength?: string | null): Promise<StorageObject> {
    ...
    return { size: Number(result.contentLength ?? contentLength ?? 0), contentType, md5: result.contentMd5 || null, fileId: result.fileId || null };
  }
  ...
  private async loadAuth(): Promise<B2Auth> {
    ...
    const response = await fetch("https://api.backblazeb2.com/b2api/v2/b2_authorize_account", { headers: { Authorization: `Basic ${credentials}` } });
  ...
  private download(auth: B2Auth, path: string) {
    return fetch(`${auth.downloadUrl}/file/${encodeURIComponent(this.env.B2_BUCKET_NAME)}/${encodeURIComponent(path).replace(/%2F/g, "/")}`, { headers: { Authorization: auth.authorizationToken } });
  }
  private async upload(auth: B2Auth, path: string, body: ReadableStream<Uint8Array>, contentType: string, contentLength?: string | null) {
    const urlResponse = await fetch(`${auth.apiUrl}/b2api/v2/b2_get_upload_url`, { method: "POST", headers: { Authorization: auth.authorizationToken, "Content-Type": "application/json" }, body: JSON.stringify({ bucketId: this.env.B2_BUCKET_ID }) });
    if (!urlResponse.ok) return urlResponse;
    const info = await urlResponse.json() as { uploadUrl: string; authorizationToken: string };
    const headers: Record<string, string> = { Authorization: info.authorizationToken, "X-Bz-File-Name": encodeURIComponent(path), "Content-Type": contentType, "X-Bz-Content-Sha1": "do_not_verify" };
  ...
  private async listAllVersions(auth: B2Auth, path: string): Promise<B2Version[] | null> {
    ...
    do {
      ...
      const response = await fetch(`${auth.apiUrl}/b2api/v2/b2_list_file_versions`, { method: "POST", headers: { Authorization: auth.authorizationToken, "Content-Type": "application/json" }, body: JSON.stringify(body) });
      if (response.status === 401) return null;
      if (!response.ok) throw new Error(`B2 list versions failed (${response.status})`);
      const result = await response.json() as any;
      for (const file of result.files || []) if (file.fileName === path) versions.push({ fileName: file.fileName, fileId: file.fileId });
  ...
  private deleteOne(auth: B2Auth, version: B2Version) {
    return fetch(`${auth.apiUrl}/b2api/v2/b2_delete_file_version`, { method: "POST", headers: { Authorization: auth.authorizationToken, "Content-Type": "application/json" }, body: JSON.stringify({ fileName: version.fileName, fileId: version.fileId }) });
  }
}
